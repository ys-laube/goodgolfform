...
describe('App SSR/static harness contract', () => {
  it('renders the Korean caddie shell to a static string without browser storage globals', () => {
    ...
    expect(renderedApp).toContain('한국형 2D 셋업 비주얼');
    expect(renderedApp).toContain('발, 공, 라이를 두 시점으로 나눠 확인합니다');
  ...
  it('renders every required Korean caddie result card category from prescription output', () => {
    ...
    expect(renderedApp).toMatch(/한국형 2D 셋업 비주얼[\s\S]*위에서 본 스탠스 \/ 공 위치[\s\S]*뒤에서 본 라이 \/ 경사/i);
  ...
  it('exposes semantic shot setup state separate from dashboard copy', () => {
    ...
    expect(caddieSessionSource).toMatch(/handedness: scenario\.handedness/);
    expect(caddieSessionSource).toMatch(/clubGroup/);
    expect(caddieSessionSource).toMatch(/ballPositionPercentRightHanded/);
    expect(caddieSessionSource).toMatch(/frontBackSlope: scenario\.stanceSlope/);
    expect(caddieSessionSource).toMatch(/sideHillRelation/);
    expect(appSource).toMatch(/<CaddieShotVisual visual=\{prescription\.shotVisual\}/);
    expect(shotVisualSource).toMatch(/data-handedness=\{visual\.handedness\}/);
    expect(shotVisualSource).toMatch(/data-club-group=\{visual\.clubGroup\}/);
    expect(shotVisualSource).toMatch(/--ball-position/);
    expect(shotVisualSource).toMatch(/data-front-back=\{visual\.frontBackSlope\}/);
    expect(shotVisualSource).toMatch(/data-side-hill=\{visual\.sideHillRelation\}/);
    expect(shotVisualSource).toMatch(/sideHillRelationLabels\[visual\.sideHillRelation\]/);
    expect(shotVisualSource).toMatch(/위에서 본 스탠스 \/ 공 위치/);
    expect(shotVisualSource).toMatch(/뒤에서 본 라이 \/ 경사/);
    expect(shotVisualSource).toMatch(/<summary>근거 보기<\/summary>/);
    expect(shotVisualSource).toMatch(/드라이버는 리드발 안쪽/);
    expect(shotVisualSource).toMatch(/shot-visual-foot/);
